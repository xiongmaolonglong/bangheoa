...
async function createClient(req, res) {
  try {
    const { name, contact_name, contact_phone, email, address, status, approval_enabled } = req.body;
...
async function createDepartment(req, res) {
  try {
    ...
    const department = await ClientDepartment.create({
      client_id: clientId,
      name,
...
async function updateDepartment(req, res) {
  try {
    ...
    const department = await ClientDepartment.findByPk(req.params.id);
    if (!department || department.client_id !== parseInt(clientId)) {
...
async function createUser(req, res) {
  try {
    ...
    // 检查手机号是否已存在
    const existing = await ClientUser.findOne({ where: { phone } });
...
async function updateUser(req, res) {
  try {
    ...
    if (!user || user.client_id !== parseInt(clientId)) {
      return error(res, '人员不存在', 404);
    }
