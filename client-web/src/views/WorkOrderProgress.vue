...
async function confirmDesign(d) {
  try {
    ...
    await api.put(`/designs/${d.id}/review`, { action: 'approve' })
...
async function rejectDesign(d) {
  try {
    ...
    await api.put(`/designs/${d.id}/review`, { action: 'reject', reject_reason: reason })
...
async function confirmAllFinance() {
  try {
    ...
    for (const f of finances.value) {
      if (!f.confirmed) {
        await api.put(`/finance/${f.id}/confirm`, { confirmed: true })
      }
    }
