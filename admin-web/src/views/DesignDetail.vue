        ...
        <div class="panel-body">
          ...
          <div v-for="mat in groupedMaterials" :key="mat.type" class="mat-group" :class="'mat-' + getMatClass(mat.type)">
            <div class="mat-head">
              ...
              <span class="mat-name">{{ materialTypeLabel(mat.type) }}<span v-if="data.declaration?.project_type" class="mat-element"> - {{ data.declaration.project_type }}</span></span>
...
async function checkImageColors() {
  ...
  console.log('开始颜色检测，要求:', requiredColorName, '共', allImages.length, '张图片')

  for (const url of allImages) {
    const dominantColor = await getDominantColor(url)
    console.log('检测图片:', url, '主色调:', dominantColor.name, dominantColor)
    ...
    console.log('颜色差异:', (diff * 100).toFixed(1), '%')
...
function fillRejectedDesignToForm() {
  console.log('fillRejectedDesignToForm 被调用', {
    designStatus: data.design?.status,
    hasFaceMapping: !!data.design?.face_mapping,
    groupedMaterialsLength: groupedMaterials.value.length
  })

  if (data.design?.status !== 'rejected') {
    console.log('不是驳回状态，跳过填充')
    return
  }
  if (!data.design?.face_mapping) {
    console.log('没有 face_mapping，跳过填充')
    return
  }
  if (groupedMaterials.value.length === 0) {
    console.log('groupedMaterials 为空，跳过填充')
    return
  }

  console.log('填充驳回设计稿到表单', data.design.face_mapping)
    ...
    for (let gi = 0; gi < mat.groups.length; gi++) {
      ...
      if (group.isUnified) {
        ...
        if (images.size > 0) {
          ...
          console.log('填充一体组合', key, designForm.unified_images[key])
        }
      } else {
        // 独立面：每个面单独映射
        for (const face of group.faces) {
          ...
          if (faceImages.length > 0) {
            ...
            console.log('填充独立面', face.label, designForm.face_images[face.label])
          }
        }
      }
    }
  ...
  console.log('填充完成', { unified_images: designForm.unified_images, face_images: designForm.face_images })
}
...
async function checkImageSizes() {
  ...
  // 检测每张图片
  console.log('开始尺寸检测，共', imageChecks.length, '张图片')
  for (const check of imageChecks) {
    const size = await getImageSize(check.url)
    console.log('检测图片:', check.label, '期望:', check.expectedW, 'x', check.expectedH, '实际像素:', size.width, 'x', size.height)
    if (!size.width || !size.height) {
      console.warn('无法获取图片尺寸，跳过:', check.url)
      continue
    }
    ...
    console.log('实际尺寸(cm):', actualW.toFixed(2), 'x', actualH.toFixed(2))
    ...
    console.log('差异:', (diffW * 100).toFixed(1), '%', (diffH * 100).toFixed(1), '%')
...
async function doSubmitDesign() {
  ...
  console.log('提交设计稿', { unified_images: designForm.unified_images, face_images: designForm.face_images, allImages })
  ...
  try {
    ...
    console.log('faceMapping', faceMapping)
