      ...
      <div>
        ...
        >
          ...
          <el-descriptions :column="2" border v-else>
            ...
            <el-descriptions-item label="元素">{{ detail.project_type || '-' }}</el-descriptions-item>
        ...
        >
          <template #header-action>
            ...
            >代录测量数据</el-button>
            <el-button
              v-if="detail.current_stage === 'measurement' && measurementData?.status === 'rejected'"
              type="danger"
              size="small"
              @click="showProxyDialog = true"
            >修改测量数据</el-button>
            <el-button
              v-if="detail.current_stage === 'measurement' && measurementData?.status === 'measured'"
              type="success"
              size="small"
...
function getBuiltInValue(key) {
  const map = {
    ...
    project_type: detail.value.project_type,
...
const measurementDataForEdit = computed(() => {
  ...
  return {
    materials: measurementData.value.materials || [],
    template_id: measurementData.value.template_id
  }
})
